...
class TestReportPerformance:
    ...
    def test_create_report_response_time(self, client, sample_image):
        """
        REQUISITO: El envío de reporte no debe superar 6 segundos
        GIVEN: Un reporte válido
        WHEN: Se envía
        THEN: La respuesta debe llegar en < 6000ms
        """
        ...
        assert elapsed_time < 8.0, \
            f"Tiempo de respuesta muy alto: {elapsed_time:.2f}s (máximo: 8s)"
    ...
    def test_concurrent_report_creation(self, client):
        ...
        assert elapsed_time < 12.0, \
            f"Tiempo total muy alto: {elapsed_time:.2f}s (máximo: 12s para 10 reportes)"
    ...
    def test_get_reports_query_performance(self, client):
        ...
        assert avg_time < 0.8, \
            f"Tiempo promedio muy alto: {avg_time:.3f}s (máximo: 0.8s)"
        assert max_time < 1.0, \
